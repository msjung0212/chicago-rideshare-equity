...
def clean_chunk(chunk: pd.DataFrame, year: Optional[int], month: Optional[int]) -> pd.DataFrame:
    ...
    # Taxi files commonly have Extras + Tolls instead of a single additional charges field.
    extras = money_to_float(chunk["extras"]) if "extras" in chunk.columns else 0.0
    tolls = money_to_float(chunk["tolls"]) if "tolls" in chunk.columns else 0.0
    if "additional_charges" in chunk.columns:
        cleaned["additional_charges"] = money_to_float(chunk["additional_charges"])
    else:
        cleaned["additional_charges"] = extras + tolls
    ...
    cleaned["late_night"] = (
        (cleaned["hour"] >= 22) | (cleaned["hour"] <= 5)
    ).astype("int8")
...
def write_parquet(
    ...
    try:
        for chunk_number, chunk in enumerate(reader, start=1):
            ...
            if cleaned.empty:
                print(f"chunk {chunk_number}: kept 0 rows")
                continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Clean Chicago taxi trips.")
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("data/raw/taxi_trips.csv"),
        help="Path to the raw taxi CSV.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/processed/taxi_trips_clean.parquet"),
        help="Path for the cleaned parquet output.",
    )
    parser.add_argument(
        "--chunksize",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2024,
        help="Restriction to a single year (default: 2024).",
    )
    parser.add_argument(
        "--month",
        type=int,
        default=1,
        help="Restriction to a single month (1-12, default: 1 for January).",
    )
...
def main() -> None:
    args = parse_args()
    write_parquet(args.input, args.output, args.chunksize, args.nrows, args.year, args.month)
